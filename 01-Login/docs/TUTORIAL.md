# 01: Add Login to your Vue 2 Application
...
Most of the defaults are fine for this tutorial, but when asked "Add Vue Router for Single Page Application development?", select "yes" as we'll be making use of that later in the tutorial.
...
Move into the directory that was created for you and run `npm install` as instructed.
...
![Auth0 Universal Login](images/universal-login.png)
...
Open the `src/views/HomeView.vue` file and modify the template to include two buttons that enable the user to log in and log out:
...
Further down the `HomeView.vue` file, modify the components script and implement the `login` and `logout` methods as follows:
...
To display the profile information, create a new file called `ProfileView.vue` in the `views` folder. Use the `this.$auth.user` property to access the user's profile data and display it on the page, as in the following example:
